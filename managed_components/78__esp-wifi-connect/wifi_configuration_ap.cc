...
void WifiConfigurationAp::StartWebServer()
{
    ...
    // Register the /hardware-test/status URI
    httpd_uri_t hardware_test_status = {
        .uri = "/hardware-test/status",
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) -> esp_err_t {
            auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
            HardwareTestStatus status;
            if (this_->hardware_test_callbacks_.get_status) {
                status = this_->hardware_test_callbacks_.get_status();
            }
            char json[160];
            snprintf(json, sizeof(json),
                "{\"supported\":%s,\"running\":%s,\"rms\":%d,\"peak\":%d,\"buffered_ms\":%d}",
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &hardware_test_status));

    // Register the /hardware-test/start URI
    httpd_uri_t hardware_test_start = {
        .uri = "/hardware-test/start",
        .method = HTTP_POST,
        .handler = [](httpd_req_t *req) -> esp_err_t {
            auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
            if (this_->hardware_test_callbacks_.on_start) {
                this_->hardware_test_callbacks_.on_start();
            }
            HardwareTestStatus status;
            if (this_->hardware_test_callbacks_.get_status) {
                status = this_->hardware_test_callbacks_.get_status();
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &hardware_test_start));

    // Register the /hardware-test/stop URI
    httpd_uri_t hardware_test_stop = {
        .uri = "/hardware-test/stop",
        .method = HTTP_POST,
        .handler = [](httpd_req_t *req) -> esp_err_t {
            auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
            if (this_->hardware_test_callbacks_.on_stop) {
                this_->hardware_test_callbacks_.on_stop();
            }
            HardwareTestStatus status;
            if (this_->hardware_test_callbacks_.get_status) {
                status = this_->hardware_test_callbacks_.get_status();
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &hardware_test_stop));
...
void WifiConfigurationAp::SetHardwareTestCallbacks(HardwareTestCallbacks callbacks)
{
    hardware_test_callbacks_ = std::move(callbacks);
}
